...
def main():
    ...
    try:
        ...
        write_json(manifest, {"repo": SOURCE, "revision": REVISION,
                              "files": files})
        # Stage metadata first so tokenizer and architecture work can proceed.
        snapshot_download(SOURCE, revision=REVISION, local_dir=destination,
                          ignore_patterns=["*.safetensors", "*.pt", "*.pdf"])
        write_json(status, {"stage": "downloading", "started": started,
                            "revision": REVISION,
        ...
        verified = []
        for item in files:
            write_json(status, {"stage": "verifying", "started": started,
                                "file": item["path"],
        ...
        result = {"stage": "complete", "repo": SOURCE, "revision": REVISION,
                  "started": started, "completed": time.time(),
                  "verified_files": len(verified),
                  "preserved": ["vision", "audio", "audio_tokenizer",
                                "embedded_mtp", "dflash"]}
        write_json(receipt, result)
        write_json(status, result)
        print(json.dumps(result), flush=True)
    except Exception as error:
        write_json(status, {"stage": "failed", "started": started,
